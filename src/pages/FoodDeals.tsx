...
const FoodDeals: React.FC<FoodDealsProps> = ({ darkMode, toggleDarkMode }) => {
  const { user } = useAuth();
          ...
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
              ...
              <div className="flex-1 flex flex-col items-center">
                ...
                <div className="font-bold text-eco-green dark:text-eco-yellow">Save the Planet</div>
                <div className="text-gray-500 dark:text-eco-yellow/70 text-sm">Help reduce food waste and support sustainable practices.</div>
              </div>
            </div>
          </div>
        </section>
        ...
        {showModal && modalDeal && (
          ...
          >
            ...
            >
              ...
              {modalDeal.image && (
                <img src={modalDeal.image} alt={modalDeal.name} className="w-full h-48 object-cover rounded-lg mb-4" />
              )}
              <h3 className="text-2xl font-bold font-playfair mb-2 text-eco-green dark:text-eco-yellow">{modalDeal.name}</h3>
              <p className="text-gray-600 dark:text-eco-yellow/80 mb-4">{modalDeal.description}</p>
              <div className="flex gap-2 mb-4 flex-wrap">
                {modalDeal.tags && modalDeal.tags.map((tag: string) => (
                  <span key={tag} className="bg-eco-green/10 text-eco-green px-2 py-1 rounded-full text-xs dark:bg-eco-yellow/20 dark:text-eco-yellow">{tag}</span>
                ))}
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={() => handleWishlist(modalDeal)}
                  className="bg-white border border-eco-green text-eco-green px-3 py-2 rounded-lg flex items-center gap-2 hover:bg-eco-green hover:text-white dark:bg-gray-800 dark:border-eco-yellow dark:text-eco-yellow dark:hover:bg-eco-yellow dark:hover:text-gray-900 transition"
                >
                  <FaHeart /> Save
                </button>
